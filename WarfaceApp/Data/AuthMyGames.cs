...
namespace MyGamesRegger.Data
{
    ...
    class AuthMyGames : AuthWarfaceAcc
    {
        ...
        public AuthMyGames(string login, string password)
        {
            ...
            cookieWarface = GetWarfaceCookie(POSTauth(login, password));
        }
        ...
        string POSTauth(string log, string pass)
        {
            ...
            IEnumerable<string> cookies = response.Headers.SingleOrDefault(header => header.Key == "Set-Cookie").Value;
            List<string> cookiess = new List<string>();
            foreach (string cookie in cookies)
            {
                cookiess.Add(cookie.Split("; ")[0]);
            }
            ...
            else if (response.StatusCode == HttpStatusCode.Found && response.Headers.Location != null)
            {
                HttpContent responseContent = response.Content;
                var str = response.Headers.Location;
                return GETauth(response.Headers.Location, cookiess);
            }
            return "";
            { }
        }
